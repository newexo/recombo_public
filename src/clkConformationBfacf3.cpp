...
bfacfProbabilitiesFromComponent::bfacfProbabilitiesFromComponent(
    bfacfProbabilitiesFromComponent::impl* implementation) : implementation(implementation)
{
}

bfacfProbabilitiesFromComponent::bfacfProbabilitiesFromComponent(const bfacfProbabilitiesFromComponent& orig) :
    implementation(orig.implementation)
{
}
...
class clkConformationBfacf3::impl
{
    ...
    void searchForRecomboSitesBetweenComps(int Sequence_type, int Recombo_type, int& Total_para_site,
                                           int& Total_anti_site, int& Para_site, int& Anti_site) //Used in mmc
    {
        sites.clear();
        //      int count = 0;
        //      initRecombo();
        ...
        for (int i = 0; i < c->nedges; i++)
        {
            ...
            for (int j = 0; j < d->nedges; j++)
            {
                ...
                if ((Sequence_type == 0 && Recombo_type == 0) || (Sequence_type == 1 && (Recombo_type == 1 ||
                    Recombo_type == 2 || Recombo_type == 3)))
                {
                    if (edgesParallelAndUnitDistant(ep, eq))
                    {
                        Total_para_site++;
                ...
                else if ((Sequence_type == 1 && Recombo_type == 0) || (Sequence_type == 0 && (Recombo_type == 1 ||
                    Recombo_type == 2 || Recombo_type == 3)))
                {
                    if (edgesAntiParallelAndUnitDistant(ep, eq))
                    {
    ...
    void searchForRecomboSitesOneComp(int minarclength, int maxarclength, int Sequence_type, int Recombo_type,
                                      int& Total_para_site, int& Total_anti_site, int& Para_site,
                                      int& Anti_site) //Used in mmc
    {
        sites.clear();
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for (int j = i + 1; j < n; j++)
            {
                ...
                if (MIN(arc1, arc2) >= minarclength && MAX(arc1, arc2) <= maxarclength)
                {
                    ...
                    if ((Sequence_type == 1 && (Recombo_type == 1 || Recombo_type == 2 || Recombo_type == 3)) || (
                        Sequence_type == 0 && Recombo_type == 0))
                    {
                        if (edgesParallelAndUnitDistant(ep, eq))
                        {
                            Total_para_site++;
...
int clkConformationBfacf3::countRecomboSites(int minarclength, int maxarclength, int Sequence_type, int Recombo_type,
                                             int& Total_para_site, int& Total_anti_site, int& Para_site, int& Anti_site)
{
    //Sanity check
    ...
    if (size() == 1)
    {
        implementation->searchForRecomboSitesOneComp(minarclength, maxarclength, Sequence_type, Recombo_type,
                                                     Total_para_site, Total_anti_site, Para_site, Anti_site);
    ...
    {
        implementation->searchForRecomboSitesBetweenComps(Sequence_type, Recombo_type, Total_para_site, Total_anti_site,
                                                          Para_site, Anti_site);
        return implementation->sites.size();
    }
    return 0;
...
void calculate_virtual_coords(float* right, float* left, int* a, int* b, int* c, int* d, int first_dir)
{
    //Diwen 02/27/2019
...
void write_to_binary_file_single_comp_inverted(std::ostream& os, EdgePtr first, EdgePtr second, float* ac, float* bd,
                                               int Numvertices)
{
    floatio floatio;
...
void perform_virtual_recombination_inverted(std::ostream& os, int recombo_type, EdgePtr first, EdgePtr second,
                                            int Numvertices)
{
    //Diwen 02/27/2019
    ...
    calculate_virtual_coords(right, left, first->start, first->next->start, second->next->start, second->start,
                             first->dir);
...
void write_to_binary_file_single_comp_direct(std::ostream& os, EdgePtr first, EdgePtr second, float* ad, float* cb,
                                             int Numvertices)
{
...
void perform_virtual_recombination_direct(std::ostream& os, int recombo_type, EdgePtr first, EdgePtr second, int comp,
                                          int Numvertices)
{
    //Diwen 02/27/2019
    /* recombo_type == 1 or 4 => do positive virtual recombo
    ...
    * d if the coordinate of head of second edge */
    ...
    float right[3], left[3];
    calculate_virtual_coords(right, left, first->start, first->next->start, second->start, second->next->start,
                             first->dir);
    if (recombo_type == 1 || recombo_type == 4) // do positive virtual recombo
    {
        if (comp == 1) // knot -> link
...
int checkLocalPositivity(EdgePtr first, EdgePtr second)
{
    //Diwen 01/17/2019
    /* return 1 or 0
...
bool clkConformationBfacf3::performRecombination(std::ostream& os, int Sequence_type, int Recombo_type,
                                                 int component_num, int n)
{
    ...
    else //coherent (directed repeat)
    {
        ...
        else // do virtual recombination
        {
            ...
            {
                // knot -> link
                if (Recombo_type == 3 || Recombo_type == 6)
                {
                    // writhe_based virtual recombo
                    int local_positivity = checkLocalPositivity(site.first, site.second);
...
bool clkConformationBfacf3::performRecombination(int n)
{
    //only used by runRecombo.cpp
    implementation->lastRecombo = n;
    clkConformationBfacf3::impl::site site = implementation->sites[n];
    //checks if the site contains parallel edges, need update ep2 after return from perform_recombination() to
    // make ep1 and ep2 parallel in the site
